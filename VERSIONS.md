...
| 10.1 | 2026-07-20 | Webテスト対策の改善: ①「はじめる」→面接/Webテストの選択画面を新設 ②選択肢を正解の近傍にクラスタ（0.4〜2.4倍に制限、100と20を混ぜない）③難易度の実質的な差別化（図表L3を多段計算＝増加率の差・上位2項目の構成比・割合のポイント差に作り直し、四則も桁をレベルで段階化） | [versions/Vol10.1_webtest-difficulty-choices.html](versions/Vol10.1_webtest-difficulty-choices.html) |
| **10.2** | 2026-07-21 | Webテストの拡張基盤整備（Fable量産への土台）。科目を単一の`WT_SUBJECTS`レジストリに集約し「生成関数を1つ書いて登録するだけで科目が増える」構造にリファクタ（科目カード・プリセット・難易度ヒント・ラベルを自動反映）。itemの型と科目追加手順を`CLAUDE.md`に明文化。図表の増加率を5〜60%に制限し非現実的な値を解消 | [versions/Vol10.2_webtest-subject-registry.html](versions/Vol10.2_webtest-subject-registry.html) ＝現在の`index.html` |
